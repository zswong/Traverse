...
class TraverseRepository(private val database: TraverseDatabase) {
     ...
     */
    suspend fun getPreviousMemories(to: MemoryEntity, count: Long): List<MemoryEntity> {
        return withContext(IO) {
            access.getPreviousMemories(to.id, count)
        }
    }
     ...
     */
    suspend fun getNextMemories(to: MemoryEntity, count: Long): List<MemoryEntity> {
        return withContext(IO) {
            access.getNextMemories(to.id, count)
        }
    }
